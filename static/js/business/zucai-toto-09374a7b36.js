...
(function () {
  ...
  var Cache = function Cache(Model) {
    //--选项缓存数据
    var cacheData = itou.sessionData.getData('__sys_ToTo_chche'); //--读取缓存数据
    itou.dataModel.config({
      key: 'toto_Cache',
      opts: {
        lottery_no: {
    ...
    cache.prototype.saveCache = function () {
      ...
      itou.outil.timmer.newTimmer({
        key: 'ToToAutoCache',
        fun: function fun() {
          itou.sessionData.setData('__sys_ToTo_chche', data);
          itou.outil.timmer.clearTimmer('ToToAutoCache');
        },
        timeout: 100
      });
    };
  ...
  var Compute = function Compute(Model) {
    ...
    compute.prototype["do"] = function () {
      ...
      itou.outil.timmer.newTimmer({
        ...
        fun: function fun() {
          self.doCompute();
          itou.outil.timmer.clearTimmer('ToTo_Compute');
        },
        timeout: 100
      });
    };
    compute.prototype.doCompute = function () {
      ...
      if (len == 14) {
        ...
        // 	var zs = element.length > 0? element.length: 1;
        // 	zhushu *= zs;
        // }, this);
        var selected = {};
        ...
        // console.table(zhushu)
        zhushu = match_computer.getCount(); //--计算单倍注数
        // console.table(zhushu)
        self.zhushu = zhushu;
      } else {
        self.isenough = false;
  ...
  var Jianpan = function Jianpan(Model) {
    ...
    self.parent = Model;
    self.isShow = false;
    self.key = null; //--监听的数据键值，文本类型，结构： "compute.beishu"
    self.num = '1';
    self.oldNum = '1'; //--修改前数据，用来与修改后数字比较，相同就不需要执行更改
    self.numMax = '1000';
    self.setNum = null;
    self.canreplace = false; //--是否允许替换数字
    self.downNum = 0; //--数字键点击次数

    self.open = function (key, maxKey) {
      //--弹出软键盘，并开始监听对应键值数据
      if (key != self.key) {
        self.num = new Function('Model', 'return Model.' + key + ';')(Model) || '1'; //--获取监听数据当前值
        if (isNaN(self.num)) {
          self.num = '1';
        }
        self.oldNum = self.num;
        self.numMax = new Function('Model', 'return Model.' + maxKey + ';')(Model) || '1'; //--获取监听数据允许最大值
        if (isNaN(self.num)) {
          self.num = '1000';
        }
        self.setNum = new Function('Model', 'code', 'Model.' + key + ' = new String(code);'); //--创建数据变更函数
      }
